    ...
    <div class="col-12">
        <div class="card">
              <div class="card-header">
                <h3 class="card-title"><span></span><a href="<?= site_url('masters/products/add_new');?>" class="btn btn-sm btn-success"><i class="fas fa-plus"></i> New </a> <button type="button" id="delete_btn" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i> Delete </button></span></h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped table-sm">
                  ...
                  <tr>
                    ...
                    <td><a href="<?= site_url('masters/products/edit_product/'.$value->id);?>" class="btn btn-sm btn-secondary"><i class="fas fa-pencil-alt"></i></a></td>
                  </tr>
                  <?php 
                    $sl_no++;
                    endforeach; ?>
    ...
    $('#delete_btn').on('click', function(){
        ...
        if(post_arr.length > 0){
            if(confirm("Do you really want to delete records?")){
                $.ajax({
                    type:"POST",
                    url : base_url + "masters/products/delete_products",
                    async:false,
                    cache :false,
                    data:{ids:post_arr},
