...
<form method="POST" action="<?= site_url('masters/nosals/update_nosal/'.$nosal->nosal_id); ?>" enctype="multipart/form-data" id="vendorsForm">
    <div class="card-body row">
        <div class="form-group col-sm-6">
            <label for="name">Name <sup class="text-danger">*</sup></label>
            <input type="text" name="name" class="form-control" id="name" placeholder="Nosal Name" value="<?= $nosal->name; ?>">
        </div>
        <div class="form-group col-sm-6">
            ...
            <select name="product_id" id="product_id" class="form-control select2">
                ...
                <?php if($products): ?>
                    ...
                    <option value="<?= $vals->id; ?>" <?= (($nosal->product_id == $vals->id )? "selected":""); ?>><?= $vals->products; ?></option>
                    <?php endforeach; ?>
                <?php endif; ?>
            </select>
        </div>
